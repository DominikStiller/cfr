...
class STYLE:
    colors_dict = {
        ...
        'wood.ringwidth': sns.xkcd_rgb['green'],
        'wood.maximumlatewooddensity': sns.xkcd_rgb['forest green'],
    ...
    markers_dict_plotly = {
        ...
        'wood.ringwidth': 'triangle-up',
        'wood.maximumlatewooddensity': 'triangle-down',
...
def plot_field_map(field_var, lat, lon, levels=50, add_cyclic_point=True,
    ...
    if mode is None:
        ...
        if ndim == 1:
            ...
            raise ValueError('Wrong shape of lat or lon!')
...
def plot_proxies(df, year=np.arange(2001), lon_col='lon', lat_col='lat', type_col='type', time_col='time',
    ...
    if plot_count:
        ...
        for ptype in df_count.keys():
            proxy_count[ptype] = df_count[ptype].sum(axis=1).to_numpy(dtype=int)
